...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.userGoal.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.user-goals.update", [$userGoal->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ ($userGoal->user ? $userGoal->user->id : old('user_id')) == $id ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('goal') ? 'is-invalid' : '' }}" name="goal_id" id="goal_id">
                    @foreach($goals as $id => $goal)
                        <option value="{{ $id }}" {{ ($userGoal->goal ? $userGoal->goal->id : old('goal_id')) == $id ? 'selected' : '' }}>{{ $goal }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" type="text" name="status" id="status" value="{{ old('status', $userGoal->status) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('mentor') ? 'is-invalid' : '' }}" name="mentor_id" id="mentor_id">
                    @foreach($mentors as $id => $mentor)
                        <option value="{{ $id }}" {{ ($userGoal->mentor ? $userGoal->mentor->id : old('mentor_id')) == $id ? 'selected' : '' }}>{{ $mentor }}</option>
                    @endforeach
                </select>
