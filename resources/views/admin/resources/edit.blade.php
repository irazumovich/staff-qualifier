    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.resources.update", [$resource->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('goal') ? 'is-invalid' : '' }}" name="goal_id" id="goal_id">
                    @foreach($goals as $id => $goal)
                        <option value="{{ $id }}" {{ ($resource->goal ? $resource->goal->id : old('goal_id')) == $id ? 'selected' : '' }}>{{ $goal }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $resource->description) }}">
