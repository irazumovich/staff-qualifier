    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.qualifications.update", [$qualification->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $qualification->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('sign') ? 'is-invalid' : '' }}" type="text" name="sign" id="sign" value="{{ old('sign', $qualification->sign) }}" required>
                @if($errors->has('sign'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('next_qualification') ? 'is-invalid' : '' }}" type="number" name="next_qualification" id="next_qualification" value="{{ old('next_qualification', $qualification->next_qualification) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $qualification->description) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('qualification_goals') ? 'is-invalid' : '' }}" name="qualification_goals[]" id="qualification_goals" multiple>
                    @foreach($qualification_goals as $id => $qualification_goals)
                        <option value="{{ $id }}" {{ (in_array($id, old('qualification_goals', [])) || $qualification->qualification_goals->contains($id)) ? 'selected' : '' }}>{{ $qualification_goals }}</option>
                    @endforeach
                </select>
