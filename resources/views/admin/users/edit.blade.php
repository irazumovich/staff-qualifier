...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="user_qualifications">{{ trans('cruds.user.fields.user_qualifications') }}</label>
                <div style="padding-bottom: 4px">
                    <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                </div>
                <select class="form-control select2 {{ $errors->has('user_qualifications') ? 'is-invalid' : '' }}" name="user_qualifications[]" id="user_qualifications" multiple>
                    @foreach($user_qualifications as $id => $user_qualifications)
                        <option value="{{ $id }}" {{ (in_array($id, old('user_qualifications', [])) || $user->user_qualifications->contains($id)) ? 'selected' : '' }}>{{ $user_qualifications }}</option>
                    @endforeach
                </select>
                @if($errors->has('user_qualifications'))
                    <span class="text-danger">{{ $errors->first('user_qualifications') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.user.fields.user_qualifications_helper') }}</span>
            </div>
            <div class="form-group">
